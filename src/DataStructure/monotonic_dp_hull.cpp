...
// 2. For any value of x, query the maximum value of k * x + b. x must be non-decreasing.
// Note:
// 1. if slope and/or query is non-increasing, change position of operation
// 2. if slope and/or query is in arbitrary order, use line_container instead which has complexity of O(log n) per operation
struct monotonic_dp_hull {
    ...
    void insert(ll k, ll b) {
        assert(lines.empty() || k > lines.back().k); // ensure slope is monotonic
        line cur{k, b};
